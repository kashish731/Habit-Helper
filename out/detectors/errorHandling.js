...
exports.runErrorHandling = runErrorHandling;
/**
 ...
 */
function runErrorHandling(document) {
    const habits = [];
    // Track promises and their catches
    const promiseCalls = new Map();
    const thenWithoutCatch = new Set();
    for (let i = 0; i < document.lineCount; i++) {
        ...
        if (/fopen\s*\(/i.test(line)) {
            ...
            if (assignMatch) {
                ...
                for (let j = i + 1; j < Math.min(i + 5, document.lineCount); j++) {
                    const checkLine = document.lineAt(j).text;
                    if (new RegExp(`if\\s*\\(\\s*!?${varName}|if\\s*\\(\\s*${varName}\\s*!=|if\\s*\\(\\s*${varName}\\s*==\\s*NULL`, 'i').test(checkLine) ||
