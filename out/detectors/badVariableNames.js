...
exports.runBadVariableNames = runBadVariableNames;
/**
...
const universalAssignmentRegex = /(?:^[\s]*(?:public|private|protected|static|final|const|volatile|var|let|const|register|auto)\b[\w\s\*]*)*?(?:[A-Za-z_$][A-Za-z0-9_$\*&\s<>]*\s+)*[*&]?\s*([A-Za-z_$][A-Za-z0-9_$]*)\s*(?:[:=]\s*)/;
...
function runBadVariableNames(document) {
    const habits = [];
    for (let i = 0; i < document.lineCount; i++) {
        ...
        if (!line)
            continue;
        if (isClassLike(line) || isFunctionSignature(line))
            continue;
        ...
        if (allowedShort.has(varName)) {
            ...
            if (inFor && ["i", "j", "k"].includes(varName))
                continue;
        }
...
//# sourceMappingURL=badVariableNames.js.map