...
function isLoopOrCondition(line: string): boolean {
  return (
    /\bfor\s*\(.*\)/.test(line) ||
    /\bwhile\s*\(.*\)/.test(line) ||
    /\bif\s*\(.*\)/.test(line) ||
...
export function runMagicNumbers(document: vscode.TextDocument): Habit[] {
  const habits: Habit[] = [];
  ...
  for (let i = 0; i < document.lineCount; i++) {
    ...
    if (isLoopOrCondition(line)) continue;
    ...
    while ((m = numberRegex.exec(raw)) !== null) {
      ...
      if (!isRepeated && !isSuspicious && !isLarge) continue;
